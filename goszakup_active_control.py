...
"""
Tender Radar KZ — isolated ACTIVE Goszakup control.

Control target:
  announcement: 17602864-1
  lot numeric prefix: 87888776
  official techspec PDF objectId: 43196583
  required PDF marker: LaserJet P1102

This script does not write to Supabase. It only performs:
  Goszakup GraphQL -> select the active control lot -> select/download PDF
  -> extract PDF text -> verify control markers -> save diagnostics.

The existing GitHub Actions workflow performs the one-row Supabase patch only
when this script exits successfully.
"""
...
CONTROL_FILE_OBJECT_ID = os.getenv("CONTROL_FILE_OBJECT_ID", "43196583").strip()
...
def choose_control_lot(lots: Iterable[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    def rank(lot: Dict[str, Any]) -> int:
        ...
        if normalize_lot(num) == target_norm:
            score += 200
        if str(lot.get("id") or "").strip() == CONTROL_FILE_OBJECT_ID:
            score += 25
        return score
...
def find_control_lot(token: str) -> Dict[str, Any]:
    # Announcement-first is deliberate: this query already returns the 10 lots
    # from announcement 17602864-1, so selection by numeric prefix is stable even
    # when the API and portal disagree on digit 3 vs Cyrillic З in the suffix.
    filters = [
...
def verify_pdf_text(text: str) -> Tuple[bool, Dict[str, bool]]:
    ...
    checks = {
        ...
        "expected_text": normalize_marker(EXPECTED_TEXT) in haystack,
    }
...
def main() -> int:
    ...
    try:
        ...
        # Do not trust only the first filename. We verify the content itself by
        # announcement, lot prefix and LaserJet P1102 markers.
        ...
        if accepted_pdf is None or accepted_file is None:
            raise RuntimeError(
                "Не найден PDF, который одновременно содержит номер объявления, "
                "префикс контрольного лота и LaserJet P1102"
            )
